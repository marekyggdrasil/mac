...
export const InteractionModeUIInfo = () => {
  ...
  if (context.state.usingAuro) {
    ...
    if (context.state.actorPublicKey) {
      return (
        <div>
          You are using <strong>PrivateKey</strong> mode as{" "}
          <MinaValue>{context.state.actorPublicKey.toBase58()}</MinaValue>.
        </div>
      );
    }
    return (
      <div>
        You are using <strong>PrivateKey</strong> mode but the key is not set.
      </div>
    );
  }
};
